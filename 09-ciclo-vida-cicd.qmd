...
## Logs como artefactos de release

En el ecosistema HILL-SE, los logs no son un subproducto efímero de la ejecución:\
constituyen **artefactos de calidad y trazabilidad**, al mismo nivel que los informes de pruebas, los resultados de cobertura o los catálogos SBOM.

Cada pipeline de integración y despliegue continuo debe **conservar los registros generados durante la verificación del software** —tests unitarios, linters, tipado, análisis de seguridad y cobertura— y **adjuntarlos como artefactos firmados** al finalizar la ejecución.

Estos artefactos de logs cumplen tres propósitos fundamentales:

1.  **Trazabilidad de calidad.**\
    Permiten reconstruir el estado del sistema en cualquier momento, vinculado al commit o versión correspondiente.

2.  **Auditoría y cumplimiento.**\
    Los logs se asocian explícitamente al SBOM (Software Bill of Materials) y al número de versión, de modo que cada release queda respaldada por evidencias verificables.

3.  **Seguridad y privacidad.**\
    Antes de publicarse, los pipelines deben ejecutar **detectores de PII o secretos** (*gitleaks*, *detect-secrets*, *trufflehog*) que garanticen que ningún log contiene información sensible o identificable.

Una vez validados, los artefactos de logs se almacenan junto a los paquetes de release o en repositorios dedicados a la trazabilidad de calidad.

### Ejemplo de configuración en GitHub Actions (referencia)

El siguiente fragmento ilustra el paso final de un pipeline que conserva los logs como artefactos asociados a la release:
...
En este esquema, los logs quedan almacenados durante 90 días, firmados y asociados al identificador de la build o release.\

Su enlace con el SBOM y la versión garantiza la **observabilidad completa del ciclo de vida**, de acuerdo con los principios de trazabilidad y calidad continua del paradigma HILL-SE.
